-- 009_gender_filtering.sql
-- Gender-aware product retrieval, one rule everywhere:
--   masculine  -> masculine + genderless rows
--   feminine   -> feminine + genderless rows
--   NULL / genderless -> no filter (unisex & non-binary profiles map here)
-- NULL product_gender is treated as genderless so untagged rows stay visible.
-- (a) match_products_semantic gains p_gender (search was unfiltered).
-- (b) ff_build_outfit_v3's strict gender match relaxed to include genderless.
-- Run in the Supabase SQL editor after 008.

-- ── 1. Search RPC gains p_gender (drop first: signature change) ─────────
...
AS $$
  ...
  WHERE f.product_embedding IS NOT NULL
    ...
    AND (
      ...
      OR lower(coalesce(f.product_gender, 'genderless')) IN ('genderless', lower(p_gender))
    )
  ORDER BY f.product_embedding <=> query_embedding
  LIMIT match_count;
$$;

-- ── 2. ff_build_outfit_v3: masculine/feminine also see genderless ───────
-- Identical to 008's definition except the gender filter in base_products.
-- Same signature and return type, so CREATE OR REPLACE (no DROP needed).
  ...
  WITH
    ...
    base_products AS (
      ...
      WHERE
        ...
        -- and feminine also see genderless (and untagged) products.
        AND (
          p_gender IS NULL
          OR p.gender_lower = 'genderless'
          OR lower(coalesce(f.product_gender, 'genderless')) IN ('genderless', p.gender_lower)
        )
        -- Price filter
        AND (
